  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 md:gap-5 justify-items-center h-auto px-5 py-5">
      {products.map((product) => {
        const currentColor = selectedColors[product.id] || "whiteGold";
        const currentImages = product.colorOptions[currentColor];

        return (
          <div
            key={product.id}
            className="relative bg-white rounded-lg w-[100%] h-auto transition-shadow duration-300 hover:shadow-custom"
          >
            {/* Heart (Wishlist) Icon */}
            <div
            ...
            <div className="flex justify-center items-center mb-2 sm:mb-3">
              <span className="font-semibold text-[#4b4537] text-[.75rem] sm:text-[.8rem] md:text-[.875rem] mr-2">
                ${currentImages.price}
              </span>
